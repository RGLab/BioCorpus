...
all <- corpusFreq::makeFreqTbl(df$value)
...
tmp <- tmp[ tmp$refinR != tmp$bestGuess, ]
tmp <- tmp[ tmp$incorrect != tmp$refinR, ]
# First, we need to make a suggestions list for our incorrect terms
# The list should have two values for each incorrect term (name)
# first is the frequency table suggestion and second is the dictionary suggestion.
# Since we are editing a freqTbl using other values in the freqTbl, it
# is not possilbe to use corpusFreq:::makeSuggList().
bw <- mapply(c, suggDf$bestGuess, suggDf$hunspell, SIMPLIFY = FALSE)
names(bw) <- incorrect
tmp <- bw[1:10]
corrWords <- corpusFreq::InteractiveFindReplace(badWords = tmp, 
